...
uniform vec2 target_dims;
uniform vec2 source_dims; // dimensions of image in the texture
...
// gets RGB pixel value from YUV 4:2:0 encoded texture
vec3 FsrEasuCF(vec2 coord) {
    float y = texture2D(u_texture, coord).x;
    float v = texture2D(uv_texture, vec2(coord.x, coord.y * 0.5 + 0.5)).x - 0.5;
    float u = texture2D(uv_texture, vec2(coord.x, coord.y * 0.5)).x - 0.5;
    // fullrange = false
    y = (y - shiftY) * scaleY;
...
void main() {
    ...
    vec2 rendersize = source_dims;
    FsrEasuCon(con0, con1, con2, con3, rendersize, rendersize, target_dims);
